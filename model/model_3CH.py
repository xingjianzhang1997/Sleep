...
4.Others：(1) 来源：《AttnSleep: An Attention-based Deep Learning Approach for Sleep Stage Classification with Single-Channel EEG》.
          (2) 原单通道的AI网络模型结构修改为三通道输入, 模型结构生成3个，让模型参数不同用。
"""
...
class AttnSleep_3CH_S2(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Linear(d_model * afr_reduced_cnn_size * 3, num_classes)
    ...
    def forward(self, x):
        ...
        x_feat_EMG = self.mrcnn_EOG(x_EMG)
        ...
        encoded_features_EMG = self.tce_EOG(x_feat_EMG)
        encoded_features_EMG = encoded_features_EOG.contiguous().view(encoded_features_EMG.shape[0], -1)
...
class AttnSleep_3CH_S2_1(nn.Module):
    """
    三通道输入,分别通过MRCNN+AFR+TCE后合并，再输入分类模块
    在AttnSleep_3CH_S2基础上，增加一层全连接层，减少三个模态的数据融合
    """
    def __init__(self):
        super(AttnSleep_3CH_S2_1, self).__init__()

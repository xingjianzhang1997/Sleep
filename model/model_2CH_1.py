...
class AttnSleep_2CH_S3(nn.Module):
    """双通道（1个EEG+1个EOG）输入,分别通过MRCNN后就合并，再输入AFR+TCE+分类模块"""
    def __init__(self):
        super(AttnSleep_2CH_S3, self).__init__()
        ...
        self.mrcnn_2CH = MRCNN_2CH(afr_reduced_cnn_size)
        attn = MultiHeadedAttention(h, d_model, afr_reduced_cnn_size)
        ff = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.tce = TCE(EncoderLayer(d_model, deepcopy(attn), deepcopy(ff), afr_reduced_cnn_size, dropout), N)
